...
/**
 * Program that annotates entities in an ontology with annotation assertions by
 * classifying class expressions provided in a configuration file.
 */
public class VFBExpressionAnnotator {
	...
	private Map<String,Set<String>> classExpressionNeoLabelMap = new HashMap<>();
	...
	private VFBExpressionAnnotator(File ontology_file, File config_file, String annotation_iri,
			boolean unique_annotations, File outfile)
	...
	private void run() throws IOException, OWLOntologyCreationException, OWLOntologyStorageException {
		...
		for (OWLEntity e : annotations.keySet()) {
			for (String s : annotations.get(e)) {
				OWLAnnotationAssertionAxiom ax = df.getOWLAnnotationAssertionAxiom(ap_anno, e.getIRI(),
						df.getOWLLiteral(s));
				axioms.add(ax);
			}
		}
	...
	private void addDynamicNodeLabels(OWLReasoner r) {
		for (String ces : classExpressionNeoLabelMap.keySet()) {
			Set<String> labels = classExpressionNeoLabelMap.get(ces);
			for (String label : labels) {
				log("Adding label " + label + " to " + ces + ".");
				try {
					...
					if (!label.isEmpty()) {
						for (OWLClass sc : getSubClasses(r, ce, false))
							addAnnotation(sc, label);
						for (OWLNamedIndividual sc : getInstances(r, ce, false))
							addAnnotation(sc, label);
						if (!ce.isAnonymous()) {
							// also annotate node itself
							addAnnotation(ce.asOWLClass(), label);
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
	...
	/**
	 * Builds a label hierarchy based on the hierarchy of label related class
	 * expressions.
	 * 
		...
		for (String ces : classExpressionNeoLabelMap.keySet()) {
			...
			for (String label : labels) {
				...
				for (String cesCompare : classExpressionNeoLabelMap.keySet()) {
					Set<String> labelsCompare = classExpressionNeoLabelMap.get(cesCompare);
